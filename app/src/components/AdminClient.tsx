...
type Tab = "dashboard" | "users" | "content" | "reports" | "profile";
...
export default function AdminClient({ mustChangePassword }: { mustChangePassword: boolean }) {
  const [activeTab, setActiveTab] = useState<Tab>(mustChangePassword ? "profile" : "dashboard");
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/users?page=${userPage}`)
      .then((r) => r.json())
      .then((d) => { setUsers(d.users ?? []); setUserTotal(d.total ?? 0); setUsersLoading(false); })
  ...
  async function handleBan(userId: string) {
    setBanningId(userId); setBanError("");
    try {
      const res = await fetch(`/api/admin/users/${userId}/ban`, { method: "POST" });
      const data = await res.json();
      if (!res.ok) setBanError(data.error);
      else setUsers((prev) => prev.map((u) => u.id === userId ? { ...u, isBanned: data.isBanned } : u));
    } catch { setBanError("Network error."); }
    finally { setBanningId(null); }
  }

  async function handleDelete(userId: string) {
    setDeletingId(userId); setBanError("");
    try {
  ...
  async function handleResolve(reportId: string) {
    ...
    try {
      ...
      if (res.ok) setReports((prev) => prev.map((r) => r.id === reportId ? { ...r, status: "RESOLVED", resolvedAt: new Date().toISOString() } : r));
  ...
  async function handleCreateAnnouncement(e: React.FormEvent) {
    ...
    try {
      const body: { title: string; message: string; type: string; expiresAt?: string } = {
        title: annForm.title, message: annForm.message, type: annForm.type,
      };
  ...
  async function handleDeactivateAnnouncement(id: string) {
    ...
    try {
      ...
      if (res.ok) setAnnouncements((prev) => prev.map((a) => a.id === id ? { ...a, isActive: false } : a));
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .adm-logout-btn:hover { border-color: #fca5a5; color: #dc2626; background: #fef2f2; }

        /* ===== TAB BAR ===== */
        .adm-tabbar {
          position: sticky; top: 65px; z-index: 90;
          background: var(--bg-navbar); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
        ...
        .adm-tab:hover { color: var(--text-heading); }
        ...
        .adm-warning-link {
          color: #92400e; font-weight: 700; text-decoration: underline;
          background: none; border: none; cursor: pointer; font-family: inherit; font-size: 14px; padding: 0;
        }
        ...
        /* ===== CHIPS ===== */
        .adm-chip { display: inline-flex; align-items: center; padding: 3px 10px; border-radius: 999px; font-size: 11px; font-weight: 600; }
        ...
        .adm-page-btn {
          padding: 6px 12px; border-radius: 8px; border: 1.5px solid var(--border-default);
        ...
        .adm-divider { border: none; border-top: 1px solid var(--border-default); margin: 20px 0; }
        ...
        /* ===== ANNOUNCEMENTS ===== */
        .ann-form { display: flex; flex-direction: column; gap: 10px; margin-bottom: 20px; }
        ...
        .ann-item { display: flex; align-items: flex-start; gap: 12px; padding: 12px 14px; border-radius: 12px; border: 1.5px solid var(--border-default); }
        ...
        .ann-success { background: #dcfce7; color: #16a34a; font-size: 13px; font-weight: 600; padding: 10px 14px; border-radius: 10px; }

        /* ===== PROFILE / PASSWORD ===== */
        .adm-profile-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; align-items: start; }
        .adm-profile-info { display: flex; flex-direction: column; gap: 12px; }
        .adm-profile-avatar {
        ...
        .adm-profile-label { font-size: 11px; font-weight: 600; color: var(--text-subtle); text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 2px; }
        .adm-profile-value { font-size: 14px; font-weight: 600; color: var(--text-heading); }
      ...
      <nav className={`adm-navbar ${scrolled ? "scrolled" : ""}`}>
        <div className="adm-navbar-inner">
          ...
          <div className="adm-nav-right">
            <ThemeToggle />
            <button className="adm-logout-btn" onClick={() => signOut({ callbackUrl: "/auth" })}>
          ...
          {TABS.map((tab) => (
            ...
            >
              ...
              {tab.id === "reports" && openReportCount > 0 && (
                <span className="adm-tab-badge">{openReportCount}</span>
              )}
            </button>
          ))}
        </div>
            ...
            <div className="adm-card">
              <div className="adm-card-header">
                <div>
                  ...
                  <div className="adm-card-subtitle">Broadcast messages via banner, email, or both</div>
                </div>
              </div>
              ...
              <form className="ann-form" onSubmit={handleCreateAnnouncement}>
                ...
                {annSuccess && <div className="ann-success">{annSuccess}</div>}
                      ...
                      <div className="ann-item-info">
                        ...
                        <div className="ann-item-meta">
                          ...
                          {ann.expiresAt && ann.isActive && (
                            <span className="ann-item-date">Expires {new Date(ann.expiresAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
                          )}
                        </div>
                      </div>
                      {ann.isActive && (
          ...
          <>
            ...
            {usersLoading ? (
              ...
              <div className="adm-stats">
                ...
                <div className="adm-stat">
                  <div className="adm-stat-value">{users.filter((u) => !u.isBanned && u.emailVerified).length}</div>
                  <div className="adm-stat-label">Active users</div>
                </div>
                <div className="adm-stat">
                  <div className="adm-stat-value">{users.filter((u) => u.isBanned).length}</div>
                  <div className="adm-stat-label">Banned</div>
                </div>
                <div className="adm-stat">
                  <div className="adm-stat-value">{users.filter((u) => !u.emailVerified).length}</div>
            ...
            <div className="adm-card">
              <div className="adm-card-header">
                <div>
                  ...
                  <div className="adm-card-subtitle">View, search, ban/unban, or delete users</div>
                </div>
              ...
              {usersLoading ? (
                ...
                <div className="adm-empty">{userSearch ? "No users match your search." : "No users yet."}</div>
              ) : (
                    ...
                    <tbody>
                      {filteredUsers.map((user) => (
                        <tr key={user.id}>
                          ...
                          <td style={{ color: "var(--text-subtle)", fontSize: 13 }}>{formatDate(user.createdAt)}</td>
                          <td>
                            {user.role !== "ADMIN" ? (
                              <div className="adm-action-cell">
                                ...
                                {deleteConfirmId === user.id ? (
                                  <>
                                    <button className="adm-delete-btn confirm" disabled={deletingId === user.id} onClick={() => handleDelete(user.id)}>
                                      {deletingId === user.id ? "..." : "Confirm"}
                                    </button>
                                    <button className="adm-delete-btn" onClick={() => setDeleteConfirmId(null)}>Cancel</button>
                                  </>
                                ) : (
                                  <button className="adm-delete-btn" onClick={() => setDeleteConfirmId(user.id)}>Delete</button>
                                )}
                              </div>
                            ) : (
              ...
              {userTotal > USER_PAGE_SIZE && (
                <div className="adm-pagination">
                  <span>{(userPage - 1) * USER_PAGE_SIZE + 1}–{Math.min(userPage * USER_PAGE_SIZE, userTotal)} of {userTotal}</span>
                  <button className="adm-page-btn" disabled={userPage === 1} onClick={() => setUserPage((p) => p - 1)}>← Prev</button>
                  <button className="adm-page-btn" disabled={userPage * USER_PAGE_SIZE >= userTotal} onClick={() => setUserPage((p) => p + 1)}>Next →</button>
                </div>
              )}
            </div>
          </>
                ...
                {contentItems.map((item) => (
                  <div key={item.id} className={`adm-cm-item ${item.isHidden ? "hidden" : ""}`}>
                    <div className="adm-cm-info">
                      <a href={item.url} target="_blank" rel="noopener noreferrer" className="adm-cm-title">{item.title}</a>
          ...
          <div className="adm-card">
            <div className="adm-card-header">
              <div>
                ...
                <div className="adm-card-subtitle">Issues submitted by users · {openReportCount} open</div>
              </div>
            </div>
          ...
          <>
            {mustChangePassword && !pwSuccess && (
              <div className="adm-warning">
                ...
                <span><strong>Security notice:</strong> You are using the default admin password. Please change it now.</span>
              </div>
            )}
            ...
            <div className="adm-card">
              <div className="adm-card-header">
                <div>
                  <div className="adm-card-title">Admin Profile</div>
                  <div className="adm-card-subtitle">Your account details and security settings</div>
                </div>
              </div>
              ...
              <div className="adm-profile-grid">
                <div className="adm-profile-info">
                  ...
                  <div>
                    <div className="adm-profile-label">Role</div>
                    <div className="adm-profile-value"><span className="adm-chip chip-admin">Administrator</span></div>
                  </div>
                  <div>
                    <div className="adm-profile-label">Session</div>
                    <div className="adm-profile-value" style={{ fontSize: 13, color: "var(--text-muted)" }}>Expires after 8 hours of inactivity</div>
                  </div>
                </div>

                <div>
                  <div style={{ fontSize: 15, fontWeight: 700, color: "var(--text-heading)", marginBottom: 16 }}>Change Password</div>
